...
namespace Stardust.Xml
{
    ...
    public class XmlBuilder
    {
        ...
        public void RegisterClass(Type elementClass)
        {
            ...
            try
            {
                newInstance = Activator.CreateInstance(elementClass);
            }
            catch (MissingMethodException exception)
            {
                throw new InvalidOperationException("Unable to instantiate " + elementClass + 
                                                    ". Perhaps it does not has a parameterless constructor? " +
        ...
        public StardustElement GetElementByName(string name)
        {
            ...
            {
                throw new InvalidOperationException("Unable to find the class for " + name + 
                                                    " Perhaps you forgot to register it with RegisterClass()?");
            }
        ...
        public void BuildFromXml(XElement xml)
        {
            ...
            StardustElement element;
            // Collect
            foreach (XElement descendant in xml.Elements())
            {
                foreach (XElement node in descendant.Elements())
                {
                    try {
